...
export async function claimLegacyAccountForWorkspace(accountId: string, workspaceId: string): Promise<boolean> { const rows = (await sql().query(`UPDATE workspaces SET account_id = $1, updated_at = now() WHERE workspace_id = $2 AND NOT EXISTS (SELECT 1 FROM workspaces existing WHERE existing.account_id = $1) RETURNING workspace_id`, [accountId, workspaceId])) as unknown as any[]; return rows.length > 0 }
