...
const dashboardKnown = await app.request(`https://mcp.onchaindiligence.com/receipts/${REFERENCE_RECEIPT.receipt_id}`, { headers: { Origin: 'https://app.onchaindiligence.com' } })
assert.equal(dashboardKnown.status, 200)
assert.equal(dashboardKnown.headers.get('access-control-allow-origin'), 'https://app.onchaindiligence.com')
assert.equal(dashboardKnown.headers.get('vary'), 'Origin')
